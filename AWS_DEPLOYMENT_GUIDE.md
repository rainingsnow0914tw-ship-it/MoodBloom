...
| 條件 | 說明 |
|------|------|
| AWS CLI 裝好 | `npm install -g aws-cli` 或從 AWS 官網安裝 |
| AWS 認證設好 | `aws configure` 填 Access Key + Secret Key |
| Claude Code 裝好 | `npm install -g @anthropic-ai/claude-code` |

有了這三樣，Claude Code 可以自動執行：

```bash
# 1. 更新 Lambda 程式碼
aws lambda update-function-code \
  --function-name moodbloom-nova-proxy \
  --zip-file fileb://lambda.zip

# 2. 更新 Lambda 環境變數
aws lambda update-function-configuration \
  --function-name moodbloom-nova-proxy \
  --environment "Variables={NOVA_API_KEY=xxx,NOVA_API_URL=xxx,NOVA_MODEL=nova-2-lite-v1}"

# 3. 上傳前端到 S3
aws s3 sync ./frontend/ s3://moodbloom-frontend-5106/ --delete

# 4. 清 CloudFront 快取
aws cloudfront create-invalidation \
  --distribution-id E2TMDPSFUMPJO9 \
  --paths "/*"
```
...
```bash
# deploy.sh — MoodBloom 一鍵部署
echo "📦 上傳前端到 S3..."
aws s3 sync ./frontend/ s3://moodbloom-frontend-5106/ --delete

echo "🔄 更新 Lambda..."
cd lambda && zip -r ../lambda.zip index.mjs && cd ..
aws lambda update-function-code --function-name moodbloom-nova-proxy --zip-file fileb://lambda.zip

echo "🧹 清 CloudFront 快取..."
aws cloudfront create-invalidation --distribution-id E2TMDPSFUMPJO9 --paths "/*"

echo "✅ 部署完成！"
echo "🌐 https://YOUR_DISTRIBUTION.cloudfront.net"
```

有了 Claude Code，你只需要說：「幫我部署 MoodBloom 最新版」，它就會自動跑完整流程。
...
|------|--------|------|
| `index.html` | S3 | 前端入口 |
| `app.js` | S3 | 前端邏輯（含 API Gateway URL） |
| `style.css` | S3 | 樣式 |
| `seed_diaries.js` | S3 | 10 篇種子日記 |
| `Celestial_Bloom.mp3` | S3 | 原創 BGM |
| `index.mjs` | Lambda | 後端 proxy（注入 API Key → Nova） |
| `.env` | 僅本機 | 本機開發用的 API Key（不上傳） |
| `start_server.js` | 僅本機 | 本機開發用的 proxy server |
